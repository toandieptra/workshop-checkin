...
const nextConfig = {
  ...
  async rewrites() {
    ...
    const apiPrefixes = ["export", "guests", "checkin", "public", "lark", "thong-ke", "registration-forms", "zbs", "zalo-agent", "zalo"];
    ...
    return [
      ...
      {
        source: "/workshops",
        destination: `${backend}/api/workshops`,
      },
      {
        source: "/workshops/:id([0-9a-fA-F-]{36})/:path*",
        destination: `${backend}/api/workshops/:id/:path*`,
      },
      {
        source: "/workshops/meta/:path*",
        destination: `${backend}/api/workshops/meta/:path*`,
      },
