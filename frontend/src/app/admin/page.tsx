...
interface Workshop {
  id: string; name: string; slug: string; event_date?: string; location?: string;
  lark_workshop_name?: string;
  last_synced_at?: string | null;
}
interface Guest {
  id: string; full_name: string; phone?: string; email?: string; company?: string;
  business_model?: string; role_title?: string; guest_type?: string; note?: string;
  party_size?: number; checkin_status: string; checked_in_at?: string;
  actual_party_size?: number;
  lark_record_id?: string | null; registered_at?: string | null; created_at?: string;
  sync_status?: string; sync_error?: string | null;
}
interface LarkWorkshop { lark_workshop_name: string; event_date?: string; location?: string; }
...
export default function AdminPage() {
  ...
  const [ng, setNg] = useState({
    full_name: "", phone: "", business_model: "", role_title: "",
    guest_type: "", party_size: 1,
  });
  ...
  // Danh sách đầy đủ, không phụ thuộc search/filter — dùng cho thống kê khối Thông tin Workshop.
  const loadAllGuests = useCallback(async (id: string) => {
    if (!id) { setAllGuests([]); return; }
  ...
  useEffect(() => { loadGuests(wid, debouncedSearch); }, [wid, debouncedSearch, loadGuests]);

  // Thống kê khối "Thông tin Workshop" phải độc lập với search/filter.
  // Nạp lại danh sách đầy đủ mỗi khi đổi workshop hoặc khi bảng khách thay đổi (sau check-in, sửa, đồng bộ...).
  useEffect(() => { loadAllGuests(wid); }, [wid, loadAllGuests]);
  useEffect(() => { if (wid) loadAllGuests(wid); }, [guests, wid, loadAllGuests]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (res && res.id) {
          if (isInitial) { isInitial = false; lastEventId = res.id; return; }
  ...
  const createGuest = async () => {
    ...
    setNg({ full_name: "", phone: "", business_model: "", role_title: "", guest_type: "", party_size: 1 });
  ...
  const doUncheckin = async (guest: Guest) => {
    try {
  ...
  const runLarkPull = async () => {
    ...
    try {
      ...
      const res = await api<any>("/lark/sync/pull?" + params.toString(), {
        method: "POST",
      });
      ...
      await loadGuests(res.workshop_id || wid, debouncedSearch);
    } catch (e: any) {
  ...
  const runLarkFull = async () => {
    ...
    try {
      ...
      const res = await api<any>("/lark/sync/full?" + params.toString(), {
        method: "POST",
      });
  ...
  const resolveConflict = async (guest: Guest, direction: "local" | "lark") => {
    try {
  ...
  // Stats — độc lập với search/filter, tính trên toàn bộ khách của workshop.
  const totalRegistered = allGuests.reduce((s, g) => s + (g.party_size || 1), 0);
  const totalCheckedIn = allGuests
    .filter((g) => g.checkin_status === "checked_in")
    .reduce((s, g) => s + (g.party_size || 1), 0);
  const totalRecords = allGuests.length;
  const checkedInRecords = allGuests.filter((g) => g.checkin_status === "checked_in").length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <section className="mb-4">
          ...
          <div className="bg-surface rounded-md border border-line p-4 mb-4 flex items-center gap-3 flex-wrap justify-between">
            <select className="border border-line rounded-sm px-3 py-2 min-w-[240px]" value={wid} onChange={(e) => setWid(e.target.value)}>
              {workshops.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
            </select>
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={runLarkWorkshopsSync} disabled={larkBusy}
              ...
              <button onClick={runLarkFull} disabled={larkBusy || !wid} className="bg-brand text-white px-3 py-2 rounded-sm text-sm disabled:opacity-50">Đồng bộ toàn bộ</button>
              <button onClick={openLark} className="border border-brand text-brand px-3 py-2 rounded-sm text-sm">Đồng bộ từ Lark</button>
              <button onClick={runLarkPush} disabled={larkBusy || !wid} className="border border-brand text-brand px-3 py-2 rounded-sm text-sm disabled:opacity-50">Đẩy lên Lark</button>
              <a href={API_URL + "/workshops/" + wid + "/export"} className="border border-line px-3 py-2 rounded-sm text-sm">Xuất CSV</a>
          ...
          {currentWorkshop && (
            <>
              {/* Main grid ~70/30 — hai cột cân bằng chiều cao */}
              <div className="grid lg:grid-cols-10 gap-4 mb-4 items-stretch">
                {/* Thông tin Workshop + KPI (7 cols) */}
                <div className="lg:col-span-7 h-full bg-surface rounded-md border border-line p-5 flex flex-col">
                  ...
                  {/* Nhóm thống kê 2×2 — giãn lấp đầy chiều cao còn lại */}
                ...
                {/* QR Check-in + Link Welcome (3 cols, xếp dọc, cân bằng chiều cao) */}
                <div className="lg:col-span-3 flex flex-col gap-4 h-full">
                  ...
                  {/* Khối Link Welcome — khung viền đỏ nổi bật */}
        ...
        <section className="bg-surface rounded-md border border-line p-4 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
            <input placeholder="Họ và tên *" className="border border-line rounded-sm px-2 py-1"
              value={ng.full_name} onChange={(e) => setNg({ ...ng, full_name: e.target.value })} />
            <input placeholder="Số điện thoại" className="border border-line rounded-sm px-2 py-1"
              value={ng.phone} onChange={(e) => setNg({ ...ng, phone: e.target.value })} />
            <input placeholder="Mô hình kinh doanh" className="border border-line rounded-sm px-2 py-1"
              value={ng.business_model} onChange={(e) => setNg({ ...ng, business_model: e.target.value })} />
            <input placeholder="Chức vụ" className="border border-line rounded-sm px-2 py-1"
              value={ng.role_title} onChange={(e) => setNg({ ...ng, role_title: e.target.value })} />
            <input placeholder="Loại khách (VIP/Speaker/Press)" className="border border-line rounded-sm px-2 py-1"
              value={ng.guest_type} onChange={(e) => setNg({ ...ng, guest_type: e.target.value })} />
            <input type="number" min={1} placeholder="Số khách" className="border border-line rounded-sm px-2 py-1"
              value={ng.party_size}
        ...
        <section className="bg-surface rounded-md border border-line">
          <div className="sticky top-0 z-30 bg-surface/95 backdrop-blur border-b border-line px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <select className="border border-line rounded-sm px-3 py-2 text-sm"
              ...
              <input className="border border-line rounded-sm px-3 py-2 text-sm min-w-[240px]"
                placeholder="Tìm tên, SĐT, công ty..." value={search}
                  ...
                  return (
                    <tr key={g.id} className={(vip ? "bg-cyan-50" : "") + " hover:bg-brand/5"}>
                      ...
                      <td className="px-3 py-3 align-top whitespace-nowrap">
                        {g.phone ? (
                          <button onClick={() => copyPhone(g.phone!)}
                            className="text-muted hover:text-brand-teal font-mono text-xs px-2 rounded min-h-[32px] flex items-center" title="Copy DT">
                            {g.phone}
                          </button>
                        ...
                        <div className="flex flex-col gap-1 items-center">
                          ...
                          {g.sync_status === "conflict" && (
                            <div className="flex gap-1 mt-1">
                              <button onClick={() => resolveConflict(g, "local")}
                                className="text-xs text-blue-600 underline">Local</button>
                              <button onClick={() => resolveConflict(g, "lark")}
                                className="text-xs text-purple-600 underline">Lark</button>
                            </div>
                          )}
                        </div>
                      </td>
                      <td className="px-3 py-3 align-top text-sm">
                        <div className="flex items-center gap-3 flex-wrap">
                          <button onClick={() => openEdit(g)}
                            className="text-brand underline min-h-[32px] flex items-center">Sửa</button>
                          <button onClick={() => toggleVip(g)}
                            className="text-muted underline min-h-[32px] flex items-center">
                            {vip ? "Bỏ VIP" : "VIP"}
                          </button>
                          <button onClick={() => delGuest(g.id)}
                            className="text-red-600 underline min-h-[32px] flex items-center">Xóa</button>
                        </div>
                      </td>
          ...
          <div className="bg-surface rounded-md border border-line p-5 w-full max-w-lg"
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <label className="col-span-2">
                ...
                <input className="border border-line rounded-sm px-2 py-1.5 w-full"
                  value={ef.full_name} onChange={(e) => setEf({ ...ef, full_name: e.target.value })} />
              </label>
              <label>
                <span className="block text-muted text-xs mb-1">Số điện thoại</span>
                <input className="border border-line rounded-sm px-2 py-1.5 w-full"
                  value={ef.phone} onChange={(e) => setEf({ ...ef, phone: e.target.value })} />
              </label>
              <label>
                <span className="block text-muted text-xs mb-1">Số vé</span>
                <input type="number" min={1} className="border border-line rounded-sm px-2 py-1.5 w-full"
                  value={ef.party_size}
              ...
              <label className="col-span-2">
                ...
                <input className="border border-line rounded-sm px-2 py-1.5 w-full"
                  value={ef.business_model} onChange={(e) => setEf({ ...ef, business_model: e.target.value })} />
              </label>
              <label>
                <span className="block text-muted text-xs mb-1">Chức vụ</span>
                <input className="border border-line rounded-sm px-2 py-1.5 w-full"
                  value={ef.role_title} onChange={(e) => setEf({ ...ef, role_title: e.target.value })} />
              </label>
              <label>
                <span className="block text-muted text-xs mb-1">Loại khách</span>
                <input className="border border-line rounded-sm px-2 py-1.5 w-full"
                  value={ef.guest_type} onChange={(e) => setEf({ ...ef, guest_type: e.target.value })} />
              </label>
              <label className="col-span-2">
                <span className="block text-muted text-xs mb-1">Ghi chú</span>
                <textarea className="border border-line rounded-sm px-2 py-1.5 w-full resize-none"
                  rows={2} value={ef.note || ""} onChange={(e) => setEf({ ...ef, note: e.target.value })} />
              </label>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setEditId(null)} disabled={editBusy}
                className="border border-line px-3 py-1.5 rounded-sm text-sm disabled:opacity-50">Hủy</button>
              <button onClick={saveEdit} disabled={editBusy || !ef.full_name}
          ...
          <div className="bg-surface rounded-md border border-line p-5 w-full max-w-lg"
            ...
            <div className="flex justify-end gap-2 mt-2">
              <button onClick={() => setLarkOpen(false)} disabled={larkBusy}
                className="border border-line px-3 py-1.5 rounded-sm text-sm disabled:opacity-50">Hủy</button>
              <button onClick={runLarkPull} disabled={larkBusy || larkLoading || !larkPick}
                className="bg-brand text-white px-3 py-1.5 rounded-sm text-sm disabled:opacity-50">
