...
export default function ColumnVisibilityMenu<Key extends string>({
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && <div id={menuId} role="menu" aria-label="Chọn cột trong bảng"
        ...
        {columns.map((column) => <label key={column.key} role="menuitemcheckbox" aria-checked={visible[column.key]}
        ...
        <button type="button" role="menuitem" onClick={() => onChange(Object.fromEntries(columns.map(({ key }) => [key, true])) as Record<Key, boolean>)}
